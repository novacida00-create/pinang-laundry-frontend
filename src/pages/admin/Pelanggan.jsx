      ...
      if (editingPelanggan) {
        const updated = allPelanggan.map(p => 
          p.no === editingPelanggan.no 
            ? { ...p, name: newPelanggan.name, phone: newPelanggan.phone, address: newPelanggan.address }
            : p
        );
      ...
      <main style={styles.main}>
        <header style={styles.header}>
          ...
          <div style={styles.headerRight}>
            ...
            <div style={styles.notifBtn} onClick={() => setShowNotifModal(true)}>🔔{unreadCount > 0 && <span style={styles.notifBadge}>{unreadCount}</span>}</div>
        ...
        <div style={styles.statsRow}>
          <StatCard label="Total Pelanggan" val={allPelanggan.length.toString()} color="#f3e8ff" iColor="#a855f7" icon="👥" growth="5.7%" />
          <StatCard label="Pelanggan Baru" val="28" color="#dcfce7" iColor="#22c55e" icon="➕" growth="12.3%" />
          <StatCard label="Aktif Bulan Ini" val={allPelanggan.filter(p => p.status === "Aktif").length.toString()} color="#e0f2fe" iColor="#0ea5e9" icon="📈" growth="8.1%" />
          <StatCard label="Priority" val="15" color="#ffedd5" iColor="#f97316" icon="⭐" growth="2.4%" />
        </div>
          ...
          <table style={styles.table}>
            ...
            <tbody>
              {paginatedPelanggan.map((p) => (
                <PelangganRow key={p.no} no={p.no} name={p.name} phone={p.phone} address={p.address} order={p.order} status={p.status} onEdit={() => handleEditPelanggan(p)} onDelete={() => handleDeletePelanggan(p.no)} />
              ))}
            </tbody>
          </table>
        ...
        <div style={styles.modalOverlay} onClick={() => setShowNotifModal(false)}>
          <div style={styles.notifModal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.notifHeader}>
              ...
              <button onClick={() => setShowNotifModal(false)} style={styles.closeBtn}>✕</button>
            </div>
...
const PelangganRow = ({ no, name, phone, address, order, status, onEdit, onDelete }) => (
  <tr style={styles.tr}>
    <td style={styles.td}>{no}</td>
    <td style={styles.td}>👤 {toTitleCase(name)}</td>
